...
class HyperXGBoost:
    ...
    _DEFAULT_TASKS = ['regression', 'classification']
    
    def __init__(self, task:str = 'regression', model_params: dict = {}):
         
        self.model_params_ = model_params
        self.task_ = task if task in _DEFAULT_TASKS else raise ValueError(f"Mentioned {self.task_} is not supported !!! Choose either 'regression' or 'classification'.")
    ...
    def train_test_split_Xy(self, X,y, test_size = 0.3, random_state = 42,**kwargs):
        X_train, X_test, y_train, y_test = train_test_split(X = X, y=y, test_size=test_size, **kwargs)
        return X_train, X_test, y_train, y_test
    
    def fit(self, X_train, y_train, X_test, y_test, study_n_trials = 100, early_stop_rounds = 10,  params= dict()):
        ...
        if self.task_ == 'regression':
            def objective(trial):
                ...
                model.fit(X_train, y_train, early_stopping_rounds=early_stop_rounds)
    ...
    def predict(self, X_test, y_test, metrics_summary: bool = True):
        
        if self.task_ == 'regression':
            tuned_model = xgboost.XGBRegressor(**study.best_params)
        elif:
            pass
